...
import socket, time, datetime, codecs
...
class N2K_send():
	...
	def Send_Engine(self, Instance, OilPressure, OilTemperature, Temperature, AlternatorPotential, FuelRate, TotalEngineHours, CoollantPressure, FuelPressure, Status, EngineLoad, EngineTorque):
		length = 26
		lPGN = 127489
		self.set_header(length,lPGN)
	...
	def Send_FluidLevel(self, Instance, SK_name, Level, Capacity):
		...
		#5:Black water
		type = 0
		if "petrol" in SK_name: type = 0
		elif "diesel" in SK_name: type = 0
		elif "lpg" in SK_name: type = 0
		elif "fresh water" in SK_name: type = 1
		elif "greywater" in SK_name: type = 2
		elif "rum" in SK_name: type = 3
		elif "holding" in SK_name: type = 5
		
		i=8
		self.data[i+0] = (Instance & 15) + (type & 15)*16
	...
	def Send_Temperature(self, Temp, SK_name):
		...
		i=8
		if 'water.temperature' in SK_name: 					source = 0
		elif 'outside.temperature' in SK_name: 				source = 1
		elif 'inside.temperature' in SK_name: 				source = 2
		elif 'engineRoom' in SK_name: 						source = 3
		elif 'heamainCabin' in SK_name: 					source = 4
		elif 'liveWell' in SK_name: 						source = 5
		elif 'baitWell' in SK_name: 						source = 6
		elif 'refrigerator' in SK_name: 					source = 7
		elif 'heating' in SK_name: 							source = 8
		elif 'dewPointTemperature' in SK_name: 				source = 9
		elif 'apparentWindChillTemperature' in SK_name: 	source = 10
		elif 'theoreticalWindChillTemperature' in SK_name:	source = 11
		elif 'heatIndexTemperature' in SK_name: 			source = 12
		elif 'freezer' in SK_name: 							source = 13
		elif 'exhaustTemperature' in SK_name: 				source = 14
		else: source = 252
