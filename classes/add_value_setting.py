...
class addvaluesetting(wx.Dialog):
	...
	def __init__(self,edit):
		...
		wx.Dialog.__init__(self, None, title=_('convert analog value to expected value of input ')+str(edit), size=(460,350))
	...
	def on_delete(self,e):
		...
		if selected==-1:
			self.ShowMessage(_('nothing selected'))
			return
	...
	def on_add(self,e):
		...
		try:
			...
			u = float(u)
			if r != float(int(r)):
				self.ShowMessage(_('adc value must be a number without dec point'))
				return				
		except:
			self.ShowMessage(_('value isn\'t a number'))
			return				
	...
	def on_graph(self,e):
		plt.plot(*zip(*self.listsave))
		plt.suptitle('settings to convert raw adc values (unlinear and/or no factor and/or no offset)\n to useable values  for input '+str(self.edit), fontsize=12)
