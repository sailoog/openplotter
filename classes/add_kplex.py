...
class addkplex(wx.Dialog):
	...
	def __init__(self,edit,extkplex):	
		...
		self.kplex_type = wx.ComboBox(panel, choices=self.kplex_type_list, style=wx.CB_READONLY, size=(80, 32), pos=(20, 55))
		...
		self.kplex_device_select = wx.ComboBox(panel, choices=self.SerDevLs, style=wx.CB_DROPDOWN, size=(140, 32), pos=(225, 55))
		...
		self.ofilter_select = wx.ComboBox(panel, choices=self.mode_ofilter, style=wx.CB_READONLY, size=(195, 32), pos=(20, 125+a))
		...
		self.name_ofilter_select = wx.ComboBox(panel, choices=self.name_ifilter_list, style=wx.CB_READONLY, size=(110, 32), pos=(305, 125+a))
	...
	def ok_conn(self,event):
		...
		if type_conn=='UDP' or type_conn=='TCP':
			if self.kplex_address.GetValue(): port_address=self.kplex_address.GetValue()
			else:
				self.ShowMessage(_('You must enter an Address.'))
				return
			if self.kplex_netport.GetValue(): bauds_port=self.kplex_netport.GetValue()
			else:
		...
		if self.ifilter_select.GetValue()==_('Ignore sentences:') and self.ifilter_sentences.GetValue()!=_('nothing'):
			filter_type='ignore'
			filtering=''
			r=self.ifilter_sentences.GetValue()
			l=r.split(',')
		...
		if self.ofilter_select.GetValue()==_('Accept only sentences:') and self.ofilter_sentences.GetValue()!=_('nothing'):
			ofilter_type='accept'
			ofiltering=''
		...
		if self.ofilter_select.GetValue()==_('Ignore sentences:') and self.ofilter_sentences.GetValue()!=_('nothing'):
			ofilter_type='ignore'
			ofiltering=''
			r=self.ofilter_sentences.GetValue()
			l=r.split(',')
		...
		self.add_kplex_out=['None',name,type_conn,in_out,port_address,bauds_port,filter_type,filtering,ofilter_type,ofiltering,'1',self.index]
