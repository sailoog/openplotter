			...
			for il in self.static_list:
				...
				for im in il:
					if im[0]==path:
						...
						im[1]=self.list_SK[-1]
						#print 'im ',im
						#print 'static list ',self.static_list[indexl][indexm]
					indexm += 1
				indexl += 1
	...
	def defvar(self):
		for i in self.PGN_list:
			if i == '126992':
				...
				self.propulsion_starboard_revolutions=self.setlist(['propulsion.starboard.revolutions',[0,0,0,0]])
			elif i == '127489':
				...
				self.propulsion_starboard_oilTemperature=self.setlist(['propulsion.starboard.oilTemperature',[0,0,0,0]])
				self.propulsion_starboard_temperature=self.setlist(['propulsion.starboard.temperature',[0,0,0,0]])
			elif i == '127505':
				...
				self.DC_Electrical_Properties_dcSource_voltage=self.setlist(['DC Electrical Properties.dcSource.voltage',[0,0,0,0]])	
				self.DC_Electrical_Properties_dcSource_current=self.setlist(['DC Electrical Properties.dcSource.current',[0,0,0,0]])
				self.DC_Electrical_Properties_dcSource_temperature=self.setlist(['DC Electrical Properties.dcSource.temperature',[0,0,0,0]])
			elif i == '128259':
				...
				self.environment_depth_belowTransducer=self.setlist(['environment.depth.belowTransducer',[0,0,0,0]])
				self.environment_depth_surfaceToTransducer=self.setlist(['environment.depth.surfaceToTransducer',[0,0,0,0]])
			elif i == '128275':
				...
				self.environment_outside_temperature=self.setlist(['environment.outside.temperature',[0,0,0,0]])
				...
				self.environment_inside_refrigerator_temperature=self.setlist(['environment.inside.refrigerator.temperature',[0,0,0,0]])
			elif i == '130316_1':
				self.propulsion_port_exhaustTemperature=self.setlist(['propulsion.port.exhaustTemperature',[0,0,0,0]])				
		
	def N2K_cycle(self, tick2):
		
		if tick2>self.cycle250:
			self.cycle250+=0.250
			if self.akt127488: self.N2K.Send_Engine_Rapid(0, self.propulsion_port_revolutions[1][2], self.N2K.empty16, self.N2K.empty8)
			...
			if self.akt128267: self.N2K.Send_Depth(self.environment_depth_belowTransducer[1][2], self.environment_depth_surfaceToTransducer[1][2])
			
		if tick2>self.cycle250_2:
			self.cycle250_2+=0.250
			if self.akt127488_1: self.N2K.Send_Engine_Rapid(1, int(self.propulsion_starboard_revolutions[1][2]), self.N2K.empty16, self.N2K.empty8)
			
			if self.akt129025: self.N2K.Send_Position_Rapid(self.navigation_position_latitude[1][2], self.navigation_position_longitude[1][2])			
			if self.akt129026: self.N2K.Send_COG_SOG(self.navigation_courseOverGroundTrue[1][2], self.navigation_speedOverGround[1][2])
			if self.akt130306: self.N2K.Send_Wind_Data(self.environment_wind_speedTrue[1][2], self.environment_wind_directionTrue[1][2])							
		if tick2>self.cycle500:
			self.cycle500+=0.500
			if self.akt127257: self.N2K.Send_Attitude(self.navigation_attitude_roll[1][2], self.navigation_attitude_pitch[1][2], self.navigation_attitude_yaw[1][2])		
			if self.akt127505: self.N2K.Send_FluidLevel(0, 'diesel', self.tank_diesel_level[1][2], self.tank_diesel_capacity[1][2])
			if self.akt130316: self.N2K.Send_Temperature(self.environment_inside_refrigerator_temperature[1][2],'refrigerator')

		if tick2>self.cycle500_2:
			self.cycle500_2+=0.500
			if self.akt127505_1: self.N2K.Send_FluidLevel(0, 'fresh water', self.tank_freshwater_level[1][2], self.tank_freshwater_capacity[1][2])
			if self.akt127489:
		...
		if tick2>self.cycle500_3:
			...
			if self.akt127505_2: self.N2K.Send_FluidLevel(0, 'greywater', self.tank_greywater_level[1][2], self.tank_greywater_capacity[1][2])
			if self.akt130316_1: self.N2K.Send_Temperature(self.propulsion_port_exhaustTemperature[1][2],'exhaustTemperature')
		if tick2>self.cycle500_4:
			self.cycle500_4+=0.500
			if self.akt127505_3: self.N2K.Send_FluidLevel(0, 'holding', self.tank_holding_level[1][2], self.tank_holding_capacity[1][2])
			if self.akt128259: self.N2K.Send_Speed(self.navigation_speedThroughWater[1][2], self.navigation_speedOverGround[1][2])
		...
		if tick2>self.cycle1000:
			...
			if self.akt127508: self.N2K.Send_Battery_Status(self.DC_Electrical_Properties_dcSource_voltage[1][2], self.DC_Electrical_Properties_dcSource_current[1][2], self.DC_Electrical_Properties_dcSource_temperature[1][2])
		if tick2>self.cycle1000_2:
			self.cycle1000_2+=1.000
			if self.akt130310: self.N2K.Send_Environmental_Parameters(self.environment_water_temperature[1][2], self.environment_outside_temperature[1][2], self.environment_outside_pressure[1][2])
			if self.akt130311: self.N2K.Send_Environmental_Parameters2(self.environment_water_temperature[1][2], self.environment_humidity[1][2], self.environment_outside_pressure[1][2])
	...
	def nmea_make(self,index):
		...
		for i in self.nmea_list[index][1]:
			...
			if type(i) is str:
				...
				value=i[4][1][2]
				if i[2]=='+': value=value+i[3]
				elif i[2]=='-': value=value-i[3]
				elif i[2]=='*': value=value*i[3]
				elif i[2]=='/': value=value/i[3]
								
				if i[1]=='x.x': value_str=str(round(value,1))
				elif i[1]=='x.xx': value_str=str(round(value,2))
				elif i[1]=='hhmmss.ss': 
					...
					if value>0:value_str='N'
					else: value_str='S'
				elif i[1]=='E/W':
					if value>0:value_str='E'
					else: value_str='W'
...
class MySK_to_Action():		
	...
	def Action_cycle(self, tick2):
		if tick2>self.cycle10:
			self.cycle10+=0.1
			for index,item in enumerate(self.triggers):
				if item[1]==-1:
					...
					#else:
					#	trigger_value_timestamp=now
					#'2016-08-15T20:10:07.411Z
					if type(item[1][1][7]) is int: trigger_value_timestamp=0
					else:
				...
				try:	
					...
					if trigger_value_timestamp:
						...
						#less than
						elif operator==3: self.Action_set(item, float(trigger_value) < data_value)
						#less than or equal to
						elif operator==4: self.Action_set(item, float(trigger_value) <= data_value)
						#greater than
						elif operator==5: self.Action_set(item, float(trigger_value) > data_value)
...
aktiv_NMEA = True
aktiv_Action = True
if SKN2K.PGN_list==[]: aktiv_N2K = False
if SKNMEA.nmea_list==[]: aktiv_NMEA = False
if SKAction.triggers==[]: aktiv_Action = False
